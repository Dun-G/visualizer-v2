...
if (window.location.href.split("/")[3].includes("?")) {
  ...
  config.circleWireframe =
    urlParams.get("circleWireframe") == "true" ? true : false;
  config.planeWireframe =
    urlParams.get("planeWireframe") == "true" ? true : false;
  config.color = parseInt(urlParams.get("color")) || 0x6904ce;
  config.colorChangeSpeed =
    parseFloat(urlParams.get("colorChangeSpeed")) || 0.00002;
  config.circleAmplitude = parseInt(urlParams.get("circleAmplitude")) || 7;
  config.circleFrequency =
    parseFloat(urlParams.get("circleFrequency")) || 0.00001;
...
var vizInit = function () {
  ...
  function play() {
    ...
    // Add event listener to the renderer's DOM element to toggle play/pause
    renderer.domElement.addEventListener("click", function (event) {
      var raycaster = new THREE.Raycaster();
      var mouse = new THREE.Vector2();

      // Calculate normalized device coordinates (-1 to +1) from click event
      mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
      mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;

      // Set raycaster's origin as camera position and direction as mouse coordinates
      raycaster.setFromCamera(mouse, camera);

      // Check for intersections with the 3D mesh ball
      var intersects = raycaster.intersectObject(ball);

      // If the 3D mesh ball is clicked, toggle play/pause
